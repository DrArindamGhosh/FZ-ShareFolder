# Freezes UI Elements when the simulation is running (CHECK TRACK FILE FOR DIFFERING TRAJ_MATS WITH CURVATURE)
# Target Acceleration can be controlled by the user
...
def play_model(client_socket, model_path="ppo_steering_model_final.h5"):
    ...
    while not done:
        ...
        next_state, reward, done, _ = self.step_simulation(client_socket, action)
            ...
            while not self.abort_flag.is_set():
                ...
                try:
                    while nLap < configured_laps:
                        ...
                        MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, targetVelocity, longAcceleration, oldLongThrust, scaling_factor, lastAvx = calculateTorqueControl(
                            tracks_df, velocity_profile_df, Vx, Vy, yawAngle, x, y, rearLHSWheelVelocity,
                            rearRHSWheelVelocity, frontLHSWheelSlipRatio, frontRHSWheelSlipRatio, rearLHSWheelSlipRatio, rearRHSWheelSlipRatio,
                            max_engine_brake_torque, differential_gain_1, driving_differential_gain, overrun_differential_gain, front_wheel_coefficient,
                            ...
                            disable_flag, velocity_error, scaling_factor_enabled, lastAvx, Kp, maxChangeAx, steering_input
                        )
