...
def refStateNow(trajectory_matrix, Vx, Vy, orientation, timestep, newSim, nLap):
    ...
    # print(f"Max Distance: {max_sdistance}")

    sdistance += V_magnitude * timestep * np.cos(orientation - phi)

    # print(f"Current Distance: {sdistance}")
...
def steering_control_output(e_phi, saturated_total_e_l, steer_limits):
    unsaturated_steering_controls = e_phi + saturated_total_e_l
    # unsaturated_steering_controls = unsaturated_steering_controls / 1000
    # steering_control_input = saturation(unsaturated_steering_controls, -.028, .028)
...
def calculate_steering_control(tracks_df, Vx, Vy, yawAngle, x, y, e_phi_gain, timestep, nLap, newSim, 
    ...
    # Steering Control
    steering_control_input = steering_control_output(e_phi, total_saturated_e_l, steer_limits)
    steering_outputs.append(steering_control_input)
    
    return steering_control_input, e_phi, saturated_e_l, lever_coords_plot, ideal_coordinates_plot, e_l, refLine_orientation, ref_distance, newSim
