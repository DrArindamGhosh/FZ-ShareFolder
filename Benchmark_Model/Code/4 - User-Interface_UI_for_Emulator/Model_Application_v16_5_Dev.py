# USED FOR OLD TRAJECTORIES AND VELOCITIES (ONE LAP TRACKS)
# SENDS ZEROES AT END OF SIMULATION
...
import os
import logging
from steer_control_v16_1_Dev import calculate_steering_control
from speed_control_v16_4_Dev import calculateTorqueControl
from model_traj_test_v5_1 import plotTraj, plot_e_phi_Values, plot_e_l_Values, plot_balance_Values, plot_Angles, plot_Torque, plot_TargetValues, plot_Scf_Values
import numpy as np
...
class ConsoleApp:
    ...
    def create_steer_control_parameters(self):
        ...
        tk.Label(self.steer_control_frame, text="Steer Control Rate:").grid(row=7, column=0, sticky="w")
        self.SCR_entry = tk.Entry(self.steer_control_frame, width=5)
        self.SCR_entry.grid(row=7, column=1, padx=5)
        self.SCR_entry.insert(0, "1")
    ...
    def create_torque_control_parameters(self):
        ...
        self.variableTD_var = tk.IntVar()
        self.variableTD_checkbox = tk.Checkbutton(self.torque_control_frame, text="Variable TD Enabled", variable=self.variableTD_var)
        self.variableTD_checkbox.grid(row=6, column=0, sticky="w", padx=5)

        tk.Label(self.torque_control_frame, text="Target Distance:").grid(row=6, column=2, sticky="w")
        self.targetDistance_entry = tk.Entry(self.torque_control_frame, width=5)
        self.targetDistance_entry.grid(row=6, column=3, padx=5)
        self.targetDistance_entry.insert(0, "0.01")
        ...
        tk.Label(self.torque_control_frame, text="Velocity Error:").grid(row=8, column=0, sticky="w")
        self.velocity_error_entry = tk.Entry(self.torque_control_frame, width=5)
        self.velocity_error_entry.grid(row=8, column=1, padx=5)
        ...
        self.scaling_factor_checkbox.grid(row=8, column=2, sticky="w", padx=5)

        # tk.Label(self.torque_control_frame, text="Torque Control Rate:").grid(row=9, column=0, sticky="w")
        # self.TCR_entry = tk.Entry(self.torque_control_frame, width=5)
        # self.TCR_entry.grid(row=9, column=1, padx=5)
        # self.TCR_entry.insert(0, "1")

        tk.Label(self.torque_control_frame, text="Initial Thrust Torque:").grid(row=9, column=0, sticky="w")
        self.Initial_Thrust_entry = tk.Entry(self.torque_control_frame, width=5)
        self.Initial_Thrust_entry.grid(row=9, column=1, padx=5)
        self.Initial_Thrust_entry.insert(0, "200")
    ...
    def get_combined_files(self):
        ...
        try:
            ...
            # traj_files = [file for file in os.listdir('17JanTraj') if file.endswith('.xlsx') or file.endswith('.xls')]
            # vel_files = [file for file in os.listdir('17JanVel') if file.endswith('.xlsx') or file.endswith('.xls')]
    ...
    def set_files(self, selected_files):
        ...
        # self.tracks_file = os.path.join('17JanTraj', traj_file)
        # self.velocity_profile_file = os.path.join('17JanVel', vel_file)
            ...
            while not self.abort_flag.is_set():
                ...
                try:
                    while nLap < configured_laps:
                        ...
                        steering_input, e_phi, saturated_e_l, levercoordsplot, idealcoordsplot, error_values, refLine_orientation, refDistance, newSim = calculate_steering_control(
                        ...
                        # if Exc_Handling != 0:
                        #     if not 0.25*(1/Vx) <= scaling_factor <= 4*(1/Vx):
                        #         print(f"Vehicle outside of scaling factor limits")
                        #         break
