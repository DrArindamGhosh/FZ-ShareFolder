...
import logging
import numpy as np
from steer_control_v15_1_Dev import calculate_steering_control
from speed_control_v16_1_Dev import calculateTorqueControl
from model_traj_test_v5 import plotTraj, plot_e_phi_Values, plot_e_l_Values, plot_balance_Values, plot_Angles, plot_Torque, plot_TargetValues, plot_Scf_Values
...
class PPOAgent:
    ...
    def build_model(self):
        ...
        action_output = layers.Dense(self.action_space, activation='tanh')(common)
...
########################################### Training and Play Functions ###########################################

def train_model(client_socket, agent, total_episodes=1000, max_steps_per_episode=1000):
    ...
    try:
        ...
        velocity_error = float(self.velocity_error_entry.get())
        Kp = float(self.Kp_entry.get())
        maxChangeAx = float(self.AxChange_entry.get())
        ...
        scaling_factor_enabled = self.scaling_factor_var.get()
        variable_TD_enabled = self.variableTD_var.get()
        targetDistance = float(self.targetDistance_entry.get())
        initial_thrust = int(self.Initial_Thrust_entry.get())
        SCR = int(self.SCR_entry.get())
        ...
        for episode in range(total_episodes):
            ...
            total_reward = 0
            step = 0
            state = reset_simulation(client_socket)
...
def play_model(client_socket, agent, model_path="ppo_steering_model_final.h5"):
    ...
    while not done:
        ...
        next_state, _, done = step_simulation(client_socket, action)
        state = next_state
        time.sleep(0.002)
...
def reset_simulation(client_socket):
    reset_signal = struct.pack('B', 1)
    client_socket.sendto(reset_signal, ("127.0.0.1", 6020))
    data, _ = client_socket.recvfrom(1024)
    state = struct.unpack('fffffff', data)
    return np.array(state)

def step_simulation(client_socket, action):
    try:
        action_data = struct.pack('f', action)
        client_socket.sendto(action_data, ("127.0.0.1", 6020))
        data, _ = client_socket.recvfrom(1024)
        state = struct.unpack('fffffff', data[:28])
...
def calculate_reward(state):
    e_phi = state[3]
    lateral_offset = np.abs(state[0]) + np.abs(state[1])
    reward = -(np.abs(e_phi) + lateral_offset)
    return reward
...
class ConsoleApp:
    def __init__(self, root):
        ...
        state_shape = (7,)
        action_space = 1
        self.agent = PPOAgent(state_shape, action_space)
        self.mode = tk.StringVar(value="Train")  # Default to Train mode
    ...
    def create_control_buttons(self):
        ...
        # tk.Label(mode_frame, text="Select Mode:").grid(row=0, column=0, sticky="w")
        self.train_radio = tk.Radiobutton(control_button_frame, text="Train Model", variable=self.mode, value="Train")
        self.play_radio = tk.Radiobutton(control_button_frame, text="Play Model", variable=self.mode, value="Play")
        self.train_radio.grid(row=0, column=1, padx=5)
        self.play_radio.grid(row=0, column=2, padx=5)
        ...
        self.train_radio.grid(row=0, column=5, padx=5, pady=5)
        self.play_radio.grid(row=0, column=6, padx=5, pady=5)
    ...
    def create_torque_control_parameters(self):
        ...
        tk.Label(self.torque_control_frame, text="Velocity Error:").grid(row=6, column=0, sticky="w")
        self.velocity_error_entry = tk.Entry(self.torque_control_frame, width=5)
        self.velocity_error_entry.grid(row=6, column=1, padx=5)
        self.velocity_error_entry.insert(0, "0")

        self.scaling_factor_var = tk.IntVar()
        self.scaling_factor_checkbox = tk.Checkbutton(self.torque_control_frame, text="Scaling Factor Enabled", variable=self.scaling_factor_var)
        self.scaling_factor_checkbox.grid(row=6, column=2, sticky="w", padx=5)

        tk.Label(self.torque_control_frame, text="Ax Rate of Change:").grid(row=7, column=0, sticky="w")
        self.Kp_entry = tk.Entry(self.torque_control_frame, width=5)
        self.Kp_entry.grid(row=7, column=1, padx=5)
        self.Kp_entry.insert(0, "1")

        tk.Label(self.torque_control_frame, text="Ax Max Change:").grid(row=7, column=2, sticky="w")
        self.AxChange_entry = tk.Entry(self.torque_control_frame, width=5)
        self.AxChange_entry.grid(row=7, column=3, padx=5)
        self.AxChange_entry.insert(0, "19")
    ...
    def run_server(self):
        ...
        self._append_to_console_output_safe("Server started and listening for incoming connections...\n")
        while not self.abort_flag.is_set():
            try:
                data, self.client_address = self.server_socket.recvfrom(1024)
                if data:
                    self._append_to_console_output_safe(f"Client connected from {self.client_address}\n")
                    self._update_connection_status_safe("Connected")
                    break
    ...
    def start_training(self):
        self.training_thread = threading.Thread(target=train_model, args=(self.server_socket, self.agent))
        self.training_thread.start()

    def start_playing(self):
        self.play_thread = threading.Thread(target=play_model, args=(self.server_socket, self.agent))
        self.play_thread.start()

    def run_backend_script(self):
        if self.mode.get() == "Train":
            self._append_to_console_output_safe("Training model...\n")
            train_model(self.server_socket, self.agent)
        elif self.mode.get() == "Play":
            self._append_to_console_output_safe("Playing saved model...\n")
            play_model(self.server_socket, self.agent)
