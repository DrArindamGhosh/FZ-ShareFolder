...
def refStateNow(trajectory_matrix, Vx, Vy, orientation, timestep):
    ...
    refDistancekm = refDistance / 1000

    refDistances.append(refDistancekm)

    # print(f"Reference Distance: " , refDistance)
    # print(f"Reference Orientation: " , refOrientation)

    return refDistance, refOrientation, refDistances
...
def AngularOffset(trajectory_matrix, refDistance, orientation, e_phi_gain):
    ...
    # Calculate the angular offset
    e_phi = phi - orientation # Equation 4.26
    # e_phi = ref_orientation - orientation # Equation 4.26
    # e_phi = orientation - phi  # Equation 4.26

    e_phi *= e_phi_gain
    # print("Track Orientation: ", phi)
...
def LateralOffset(r, l, orientation, gains, lower_limits, upper_limits):
    ...
    # print(total_e_l)
    saturated_total_e_l = saturation(total_e_l, -25, 25)
...
def steering_control_output(e_phi, saturated_total_e_l):
    ...
    steering_control_input = saturation(unsaturated_steering_controls, -.040, .040)
...
def calculate_steering_control(tracks_df, Vx, Vy, yawAngle, x, y, e_phi_gain, timestep):
    ...
    # Reference State Now
    ref_distance, ref_orientation, refDistances = refStateNow(trajectory_matrix, Vx, Vy, yawAngle, timestep)
    ...
    e_l_upper_limits = np.array([10, 10, 2, 2.75, 3, 2.75, 2.25, 1.7])
    e_l_lower_limits = np.array([-10, -10, -2, -2.75, -3, -2.75, -2.25, -1.7])
