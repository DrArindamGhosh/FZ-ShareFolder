...
def compute_brake_pid(error, dt, Kp, Ki, Kd, integral, prev_error, max_brake_pressure):
    """ Compute the PID correction """
    P = Kp * abs(error)
    ...
    bPressure = P + I + D
    bPressure = np.clip(bPressure, 0, max_brake_pressure)
...
def calculatePedalPercent(lap_df, vCar_Profile_df, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
                          ...
                          max_drive_torque, max_brake_torque, futureTD, StopFlag, rAccel, Dil_Mode_enabled, max_brake_pressure):
            ...
            if velocity_error < 0:
                if Dil_Mode_enabled == True:
                    if futureVelocity >= targetVelocity:
                        ...
                        brakePressure, brake_integral, brake_prev_error, brake_P, brake_I, brake_D = compute_brake_pid(velocity_error, timestep, brake_Kp, brake_Ki, brake_Kd, brake_integral, brake_prev_error, max_brake_pressure)
                        rAccel = 0
                        throttle_P = 0
                        throttle_I = 0
                        throttle_D = 0
