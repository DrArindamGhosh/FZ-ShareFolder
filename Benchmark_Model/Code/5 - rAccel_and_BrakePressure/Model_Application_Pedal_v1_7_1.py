# Tkinter and graph viewing changes added
...
class ConsoleApp:
    ...
    def create_bottom_buttons(self):
        ...
        tk.Button(self.bottom_button_frame, text="Show Graphs", command=self.show_graphs_from_folder).grid(row=0, column=0, padx=5, pady=5)
    ...
    def show_graphs_from_folder(self):
        ...
        graph_files = [file for file in os.listdir(folder_path) if file.endswith('.dill')]
        if not graph_files:
        ...
        listbox = tk.Listbox(control_frame, selectmode=tk.MULTIPLE, height=10, width=50)
        listbox.pack(pady=5)

        def populate_listbox():
            current_selection = listbox.curselection()
            listbox.delete(0, tk.END)
            new_files = [f for f in os.listdir(folder_path) if f.endswith('.dill')]
        ...
        def display_selected_graphs():
            ...
            # Clear previous plots (but not controls)
            ...
            for file in selected_files:
                try:
                    ...
                    with open(graph_path, 'rb') as f:
                        fig = dill.load(f)
                    fig.canvas.manager.window.deiconify()  # Open the full interactive window
                except Exception as e:
        ...
        tk.Button(button_row, text="Refresh List", command=populate_listbox).pack(side="left", padx=5)
