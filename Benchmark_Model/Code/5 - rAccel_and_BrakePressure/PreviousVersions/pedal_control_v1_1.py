...
def calculate_zero_pedal(Vx, Vy, vCarData, trajectory_matrix, ref_Distance, zero_pedal_distance, nLap):

    vCar = np.sqrt(Vx ** 2 + Vy ** 2)
    zero_pedal_new_distance = ref_Distance + zero_pedal_distance
    ...
    if nLap > 0:
        max_sdistance = np.max(trajectory_matrix['sDistance2'])
        velocity_interpolator = interp1d(vCarData['sDistance2'], vCarData['vCar2'], kind='linear', fill_value="extrapolate")
    future_Velocity = velocity_interpolator(zero_pedal_new_distance)
    future_Velocity = future_Velocity / 3.6
...
def calculatePedalPercent(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
                          ...
                          max_drive_torque, max_brake_torque, max_long_slip, slip_constant, zero_pedal_distance, Dil_Mode_enabled):
    ...
    future_velocity = calculate_zero_pedal(Vx, Vy, vCarData, trajectory_matrix, ref_distance, zero_pedal_distance, nLap)

    if velocity_error >= 0:
        if Dil_Mode_enabled == True:
            if future_velocity > targetVelocity or Vx < 20:
    ...
    if velocity_error < 0:
        if Dil_Mode_enabled == True:
            if future_velocity < targetVelocity:
