...
from pedal_control_v1_5 import calculatePedalPercent
from model_traj_test_v6 import (
    plotTraj, plot_e_phi_Values, plot_e_l_Values, plot_balance_Values, plot_Angles,
    plot_Torque, plot_TargetValues, plot_PedalValues, plot_nMGU_Values,
    plot_Throttle_PID_Values, plot_Brake_PID_Values, plot_vCar_error_Values, plot_refDistances
)
import numpy as np
...
def resample_uniformly(data, distance_col, x_col, y_col, orientation_col, num_points):
    ...
    uniform_distances = np.linspace(
        data[distance_col].min(),
        data[distance_col].max(),
        num_points
    )
    ...
    interpolated_orientation = np.interp(
        uniform_distances,
        data[distance_col],
        data[orientation_col]
    )
...
def generate_driving_line_DiL(tracks_df, yawAngle):
    ...
    sDistance, x, y, orientation = resample_uniformly(
        tracks_df, 'sDistance', 'x', 'y', 'Orientation', num_points=1500
    )
    sDistance2, x2, y2, orientation2 = resample_uniformly(
        tracks_df, 'sDistance2', 'x2', 'y2', 'Orientation2', num_points=1500
    )
    ...
    if clockwise_value < 0:
        tracks_df['Orientation2'] = (
            tracks_df['Orientation2']
            - tracks_df.at[0, 'Orientation2']
            + tracks_df['Orientation'].dropna().iloc[-1]
            + 2 * np.pi
        )
        clockwise = True
    else:
    ...
    combined_wrap = pd.concat(
        [wrap_df.reset_index(drop=True), wrap_lap2.reset_index(drop=True)],
        axis=1
    )
...
def generate_driving_line_Dev(tracks_df, initial_x, initial_y, yawAngle):
    ...
    for i in range(1, len(tracks_df)):
        ...
        tracks_df.at[i, 'x'] = tracks_df.at[i - 1, 'x'] + delta_distance * np.cos(
            tracks_df.at[i, 'Orientation']
        )
        tracks_df.at[i, 'y'] = tracks_df.at[i - 1, 'y'] + delta_distance * np.sin(
            tracks_df.at[i, 'Orientation']
        )
    ...
    for i in range(1, len(tracks_df)):
        ...
        tracks_df.at[i, 'x2'] = tracks_df.at[i - 1, 'x2'] + delta_distance * np.cos(
            tracks_df.at[i, 'Orientation2']
        )
        tracks_df.at[i, 'y2'] = tracks_df.at[i - 1, 'y2'] + delta_distance * np.sin(
            tracks_df.at[i, 'Orientation2']
        )
...
class ConsoleApp:
    def __init__(self, root):
        ...
        self.refresh_task = TkRepeatingTask(
            self.scrollable_frame, self._refresh_console, 2500
        )
    ...
    def create_ip_port_input(self):
        ...
        self.start_server_button = tk.Button(
            ip_port_frame,
            text="Start Server & Listen for Incoming Connections",
            bg="light yellow",
    ...
    def create_connection_status(self):
        self.connection_status = tk.Label(
            self.scrollable_frame, text="Status: Disconnected", fg="red"
        )
    ...
    def create_control_buttons(self):
        ...
        self.start_sim_button = tk.Button(
            button_row, text="START SIM", bg="green", command=self.start_simulation
        )
        self.stop_button = tk.Button(
            button_row, text="STOP SIM", bg="red", command=self.stop_simulation, state="disabled"
        )
        self.reset_button = tk.Button(
            button_row, text="RESET SIM", bg="light blue", command=self.reset_simulation_button
        )
    ...
    def create_socket_output(self):
        self.socket_output = tk.Text(
            self.scrollable_frame, height=6, width=50, bg="black", fg="green"
        )
    ...
    def create_parameter_buttons(self):
        ...
        tk.Label(self.param_frame, text="Parameter Selection:").grid(
            row=0, column=1, columnspan=3, sticky="ew", pady=5
        )

        tk.Label(self.param_frame, text="Select Track and Velocity Profile:").grid(
            row=1, column=1, sticky="w", padx=5
        )
        self.tracks_var = tk.StringVar(self.param_frame)
        self.tracks_var.set("Select Tracks")
        self.tracks_dropdown = tk.OptionMenu(
            self.param_frame, self.tracks_var, *self.get_combined_files(), command=self.set_files
        )
        ...
        tk.Label(self.param_frame, text="Velocity Percentage (%):").grid(
            row=2, column=1, sticky="w", padx=5
        )
        self.num_Velocity_var = tk.StringVar(self.param_frame)
        self.num_Velocity_var.set("70")
        self.num_Velocity_dropdown = tk.OptionMenu(
            self.param_frame, self.num_Velocity_var,
            *["70", "80", "85", "90", "92", "94", "96", "98", "100"]
        )
        ...
        self.unlimited_laps_checkbox = tk.Checkbutton(
            self.param_frame, text="Unlimited Laps", variable=self.unlimited_laps_var
        )
        ...
        tk.Label(self.param_frame, text="Number of Laps:").grid(
        ...
        self.DiL_Mode_checkbox = tk.Checkbutton(
            self.param_frame, text="DiL Mode", variable=self.DiL_Mode_var
        )
        ...
        self.invert_coords_checkbox = tk.Checkbutton(
            self.param_frame, text="Invert X and Y", variable=self.invert_coords_var
        )
    ...
    def create_steer_control_parameters(self):
        ...
        tk.Label(self.steer_control_frame, text="Steer Control Parameters:").grid(
            row=0, column=0, columnspan=4, sticky="ew"
        )

        tk.Label(self.steer_control_frame, text="Angular Offset Gain:").grid(
        ...
        tk.Label(self.steer_control_frame, text="Lateral Offset Gains:").grid(
        ...
        tk.Label(self.steer_control_frame, text="Individual Lateral Offset Saturations:").grid(
        ...
        tk.Label(self.steer_control_frame, text="Total Lateral Offset Saturations:").grid(
        ...
        tk.Label(self.steer_control_frame, text="Steering Control Saturations:").grid(
    ...
    def create_force_input_buttons(self):
        ...
        tk.Label(self.force_input_frame, text="Force Driver Inputs:").grid(
            row=0, column=0, columnspan=4, sticky="ew"
        )

        tk.Label(self.force_input_frame, text="Steering Angle:").grid(
            row=1, column=0, sticky="w"
        )
    ...
    def create_pedal_control_parameters(self):
        ...
        tk.Label(self.pedal_control_frame, text="Pedal Control Parameters:").grid(
            row=0, column=0, columnspan=4, sticky="ew"
        )

        tk.Label(self.pedal_control_frame, text="Throttle Kp:").grid(
            row=1, column=0, sticky="w"
        )
        ...
        tk.Label(self.pedal_control_frame, text="Throttle Ki:").grid(
            row=2, column=0, sticky="w"
        )
        ...
        tk.Label(self.pedal_control_frame, text="Throttle Kd:").grid(
            row=3, column=0, sticky="w"
        )
        ...
        tk.Label(self.pedal_control_frame, text="Brake Kp:").grid(
        ...
        tk.Label(self.pedal_control_frame, text="Brake Ki:").grid(
        ...
        tk.Label(self.pedal_control_frame, text="Brake Kd:").grid(
        ...
        tk.Label(self.pedal_control_frame, text="Future TD:").grid(
    ...
    def create_torque_control_parameters(self):
        ...
        tk.Label(self.torque_control_frame, text="Torque Control Parameters:").grid(
            row=0, column=0, columnspan=4, sticky="ew"
        )

        tk.Label(self.torque_control_frame, text="Max Engine Brake Torque:").grid(
            row=1, column=0, sticky="w"
        )
        ...
        tk.Label(self.torque_control_frame, text="Differential Gain 1:").grid(
            row=1, column=2, sticky="w"
        )
        ...
        tk.Label(self.torque_control_frame, text="Driving Differential Gain:").grid(
            row=2, column=0, sticky="w"
        )
        ...
        tk.Label(self.torque_control_frame, text="Overrun Differential Gain:").grid(
            row=2, column=2, sticky="w"
        )
        ...
        tk.Label(self.torque_control_frame, text="Max Drive Torque:").grid(
            row=3, column=0, sticky="w"
        )
        ...
        tk.Label(self.torque_control_frame, text="Max Brake Torque:").grid(
            row=3, column=2, sticky="w"
        )
        ...
        ToolTip(
            self.max_engine_brake_torque_entry,
            "*OFFLINE TESTING ONLY* Maximum torque applied by engine braking"
        )
        ToolTip(
            self.differential_gain_1_entry,
            "*OFFLINE TESTING ONLY* Gain for balancing torque between rear wheels"
        )
        ToolTip(
            self.driving_differential_gain_entry,
            "*OFFLINE TESTING ONLY* Gain for distributing torque while accelerating"
        )
        ToolTip(
            self.overrun_differential_gain_entry,
            "*OFFLINE TESTING ONLY* Gain for differential behavior during deceleration"
        )
        ToolTip(
            self.max_drive_torque_entry,
            "Maximum driveshaft torque the V3M is allowed to deliver"
        )
        ToolTip(
            self.max_brake_torque_entry,
            "Maximum brake torque the V3M is allowed to apply"
        )

    def update_force_dev_fields_visibility(self):
        if self.DiL_Mode_var.get() == 1:
            # Hide Dev labels and entries
    ...
    def create_live_data_frame(self):
        ...
        # Span all four columns above (steer, torque, pedal, force input)
        self.live_data_frame.grid(row=5, column=0, columnspan=4, sticky="nsew", pady=5)
        self.live_data_frame.grid_remove()

        tk.Label(self.live_data_frame, text="Live Data:").grid(
            row=0, column=0, columnspan=4, sticky="ew"
        )

        # Speed and Lap labels (existing)
        self.speed_label_var = tk.StringVar(value="Speed: 0 km/h")
        tk.Label(
            self.live_data_frame, textvariable=self.speed_label_var, font=("Arial", 12)
        ).grid(row=1, column=0, sticky="w")

        self.lap_label_var = tk.StringVar(value="Lap: 0")
        tk.Label(
            self.live_data_frame, textvariable=self.lap_label_var, font=("Arial", 12)
    ...
    def update_speed_label(self, Vx, Vy):
        speed = np.sqrt(Vx**2 + Vy**2)
        speed_kmh = speed * 3.6
        self.speed_label_var.set(f"Speed: {speed_kmh:.2f} km/h")

    def create_console_output(self):
        self.console_output = tk.Text(
            self.scrollable_frame, height=23, width=50, bg="black", fg="white"
        )
    ...
    def create_bottom_buttons(self):
        ...
        tk.Button(
            bottom_button_frame, text="Show Graphs", command=self.show_graphs_from_folder
    ...
    def get_combined_files(self):
        ...
        try:
            traj_files = [
                file for file in os.listdir('Trajectories')
                if file.endswith('.csv') or file.endswith('.xls')
            ]
            vel_files = [
                file for file in os.listdir('Velocities')
                if file.endswith('.csv') or file.endswith('.xls')
            ]
            combined_files = [
                f"{traj_file} | {vel_file}"
                for traj_file, vel_file in zip(traj_files, vel_files)
            ]
    ...
    def show_graphs_from_folder(self):
        ...
        display_button = tk.Button(
            graphs_window, text="Display Graph", command=display_selected_graph
        )
        display_button.pack()
    ...
    def run_backend_script(self):
        try:
            ...
            def reset_variables():
                nonlocal steering_input, MDriveFL, MDriveFR, MDriveRL, MDriveRR
                nonlocal MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR
                    ...
                    while nLap < configured_laps:
                        ...
                        if i:
                            ...
                            try:
                                ...
                                expected_length = 17 * 4  # 17 floats, each 4 bytes
                                ...
                                if received_length == expected_length:
                                    ...
                                    speed = abs(Vx) if abs(Vx) > abs(Vy) else abs(Vy)
                                    speed_kmh = speed * 3.6
                                    self.speed_current = speed_kmh
                        ...
                        MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, rAccel, brakePressure, throttle_integral, throttle_prev_error, brake_integral, brake_prev_error, targetVelocity, t_P, t_I, t_D, b_P, b_I, b_D, velocity_error = calculatePedalPercent(
                            tracks_df, velocity_profile_df, Vx, Vy, timeStep, refDistance, nLap,
                            throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
                            brake_Kp, brake_Ki, brake_Kd, brake_integral, brake_prev_error, nMGU,
                            WheelVelocityRL, WheelVelocityRR, max_engine_brake_torque,
                            differential_gain_1, driving_differential_gain, overrun_differential_gain,
                            max_drive_torque, max_brake_torque, futureTD, Dil_Mode_enabled
                        )
            ...
            if lever_coordinates and ideal_coordinates:
                plot_and_store(
                    self.figures, plotTraj,
                    base_name, tracks_df,
                    x_positions, y_positions,
                    levercoordsplot, idealcoordsplot,
                    error_values, current_time
                )
