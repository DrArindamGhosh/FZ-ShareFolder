...
# def calculateTargetAcceleration(targetVelocity, Vx, Vy, dt):
    ...
    # targetAvx, brake_integral, brake_prev_error = compute_brake_pid(velocity_error, timestep, Kp, Ki, Kd, brake_integral, brake_prev_error)
...
def calculateBrakeTorque(torque, rearLHSWheelVelocity, rearRHSWheelVelocity):
    ...
    # print(f"Front Brake FL: {MBrakeFL} = -{residualForce} * (({frontRadii} * {rearRadii} * {frontBrakeCoeff})/({rearRadii} * {frontBrakeCoeff} + {frontRadii} * {rearBrakeCoeff}))")
    MBrakeFR = -residualForce * ((frontRadii * rearRadii * frontBrakeCoeff)/(rearRadii * frontBrakeCoeff + frontRadii * rearBrakeCoeff))
    # print(f"Front Brake FR: {MBrakeFR} = -{residualForce} * (({frontRadii} * {rearRadii} * {frontBrakeCoeff})/({rearRadii} * {frontBrakeCoeff} + {frontRadii} * {rearBrakeCoeff}))")
    ...
    # print(f"Rear Brake RL: {MBrakeRL} = {maxEngineBrakeTorque} * {gearRatioRear} - {residualForce} * (({frontRadii} * {rearRadii} * {frontBrakeCoeff})/({rearRadii} * {frontBrakeCoeff} + {frontRadii} * {rearBrakeCoeff}))")
    ...
    # print(f"Rear Brake RR: {MBrakeRR} = {maxEngineBrakeTorque} * {gearRatioRear} - {residualForce} * (({frontRadii} * {rearRadii} * {frontBrakeCoeff})/({rearRadii} * {frontBrakeCoeff} + {frontRadii} * {rearBrakeCoeff}))")
...
def calculatePedalPercent(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
    ...
    if velocity_error < 0:
        if Dil_Mode_enabled == True:
            ...
            # targetAvx = calculateTargetAcceleration(targetVelocity, Vx, Vy, timestep)
            targetAvx, brake_integral, brake_prev_error, brake_P, brake_I, brake_D = compute_brake_pid(velocity_error, timestep, brake_Kp, brake_Ki, brake_Kd, brake_integral, brake_prev_error)
            prev_bP = brake_P
            prev_bI = brake_I
            prev_bD = brake_D
