...
class ConsoleApp:
    ...
    # def create_map_frame(self):
    #     self.map_frame = tk.Frame(self.scrollable_frame, bd=2, relief=tk.RIDGE)
    #     self.map_frame.grid(row=7, column=1, sticky="nsew", pady=5)
    #     self.map_frame.grid_remove()

    #     tk.Label(self.map_frame, text="Track Map:", font=("Arial", 12)).grid(row=0, column=0, columnspan=4, sticky="ew")

    #     # Canvas with improved appearance
    #     self.map_canvas = tk.Canvas(self.map_frame, bg="#222222", width=350, height=200, highlightthickness=1, highlightbackground="#444444")
    #     self.map_canvas.grid(row=1, column=0, columnspan=4, padx=5, pady=5)
    ...
    #     for x, y in zip(x_positions, y_positions):
    #         canvas_x = x * scale + self.map_x_offset
    #         canvas_y = canvas_height - (y * scale + self.map_y_offset)
    #         coords.extend([canvas_x, canvas_y])

    #     # Draw smooth blue track line
    #     if self.track_line_id:
    #         self.map_canvas.delete(self.track_line_id)
    #     self.track_line_id = self.map_canvas.create_line(*coords, fill="#00AFFF", width=3, smooth=True)

    #     # Create car circle with nice style
    #     if self.car_circle_id is None:
    #         self.car_circle_id = self.map_canvas.create_oval(0, 0, 0, 0, fill="#FF3333", outline="#FFFFFF", width=2)
