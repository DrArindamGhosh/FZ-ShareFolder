...
#########################################################################################################################################################
def calculateTargetVelocityError(Vx, Vy, vCarData, trajectory_matrix, targetDistance, nLap):
    max_speed = 65.2777778
...
ThrottleX = np.array([0, 7, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77, 84, 91, 98, 100])
rpmY = np.array([0, 133.3, 266.7, 400, 533.3, 666.7, 823, 960, 1066.7, 1200, 1333.3, 1466.7, 1600, 1733.3, 1866.7, 2000])
TorqueZ = np.array([
    [0, 102.7, 279.3, 510.4, 831.8, 1224.1, 1635.5, 1932.6, 2198, 2436.5, 2652.9, 2852.4, 3040.3, 3237.7, 3589.2, 3589.2],
    [0, 102.7, 279.3, 510.4, 831.8, 1224.1, 1635.5, 1932.6, 2198, 2436.5, 2652.9, 2852.4, 3040.3, 3237.7, 3589.2, 3589.2], 
    [0, 102.7, 279.3, 510.4, 831.8, 1224.1, 1635.5, 1932.6, 2198, 2436.5, 2652.9, 2852.4, 3040.3, 3237.7, 3589.2, 3589.2],
...
# Interpolation function (RPM first, then Throttle!)
torque_interp = RectBivariateSpline(rpmY, ThrottleX, TorqueZ)

# Inversion function
def get_throttle_for_torque_and_speed(target_torque, rpm):
    def objective(throttle):
        estimated_torque = torque_interp(rpm, throttle)[0][0]
        return abs(estimated_torque - target_torque)

    result = minimize_scalar(objective, bounds=(0, 100), method='bounded')
    
    if result.success:
        return result.x  # throttle percentage
    else:
...
# rpm = 2000
# desired_torque = 40
# throttle = get_throttle_for_torque_and_speed(desired_torque, rpm)

    # throttle_values = ThrottleX
    # torque_at_rpm = []

    # for i in range(len(throttle_values)):
    #     rpm_column = TorqueZ[i, :]
    #     interp = interp1d(rpmY, rpm_column, kind='linear', fill_value="extrapolate")
    #     torque_at_rpm.append(interp(target_rpm))

    # torque_at_rpm = np.array(torque_at_rpm)

    # # Step 2: Now reverse-interpolate to find throttle that gives the target torque
    # # Ensure that the torque array is sorted for interpolation
    # sorted_indices = np.argsort(torque_at_rpm)
    # sorted_torques = torque_at_rpm[sorted_indices]
    # sorted_throttles = throttle_values[sorted_indices]

    # # Check bounds
    # if not (sorted_torques[0] <= target_torque <= sorted_torques[-1]):
    #     print(f"Target torque {target_torque} is out of bounds ({sorted_torques[0]}, {sorted_torques[-1]})")
    #     return np.nan  # Or clamp, or extrapolate

    # throttle_interp = interp1d(sorted_torques, sorted_throttles, kind='linear', fill_value="extrapolate")
    # print(f"Interpolated throttle for target torque {target_torque} at {target_rpm} RPM: {throttle_interp(target_torque)}")
    # return throttle_interp(target_torque)

def ff_controller(vCarError, delta_throttle, P, I, timeStep , prev_integral, prev_output):
    def rate_limiter(input_signal, prev_output, timeStep , rising_rate=12.5, falling_rate=-7.5):
        """
        Recreates Simulink's Rate Limiter block.
        """
        max_step_up = rising_rate * timeStep
     
        max_step_down = falling_rate * timeStep
     

        delta = input_signal - prev_output

        if delta > max_step_up:
            return prev_output + max_step_up
        elif delta < max_step_down:
            return prev_output + max_step_down
        else:
            return input_signal

    """ PI controller for velocity """
    # Update integral
    integral = prev_integral + timeStep * vCarError
    
    # PI control in velocity form
    delta_output = P * (vCarError - delta_throttle) + I * timeStep * vCarError
    output = prev_output + delta_output
    
    # Apply saturation
    output = max(min(output, 25), -25)

    output = rate_limiter(output, prev_output, timeStep)

    return output, integral

def smooth_throttle(rAccels, polyorder=3):
    n = len(rAccels)
    window = min(101, n if n % 2 == 1 else n - 1)
    if window >= polyorder + 2:
        return savgol_filter(rAccels, window_length=window, polyorder=polyorder)
    else:
        # Not enough data yet, return raw or lightly smoothed
        return rAccels 
...
def calculatePedalPercent(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
    ...
    t_targetVelocity, t_velocity_error = calculateTargetVelocityError(Vx, Vy, vCarData, trajectory_matrix, targetDistance, nLap)
    # print("Velocity Error: ", t_velocity_error)
    longAcceleration = calculateTargetAcceleration(t_targetVelocity, Vx, Vy)
    # print("Longitudinal Acceleration: ", longAcceleration)
    longThrust = calculateLongitudinalThrust(longAcceleration)
    longThrust = saturation(longThrust, -3500, 3500)
    # print("Longitudinal Thrust: ", longThrust)

    if longThrust >= 0:
        if Dil_Mode_enabled == True:
            rAccel = get_throttle_for_torque_and_speed(longThrust, nMGU)
            rAccel = saturation(rAccel, 0, 100)
            rAccels.append(rAccel)
            delta_throttle = previous_rAccel - rAccel
            ...
            driver_command = ff_controller(velocity_error, delta_throttle, throttle_Kp, throttle_Ki, timestep, prev_integral, prev_output)
            rAccel += driver_command
            previous_rAccel = rAccel
        ...
        if Dil_Mode_enabled == False:
            rAccel = get_throttle_for_torque_and_speed(longThrust, nMGU)
            # print("Raw rAccel: ", rAccel)
            rAccel = saturation(rAccel, 0, 100)
            rAccels.append(rAccel)
            rAccels = smooth_throttle(rAccels, polyorder=3)
            rAccel = rAccels[-1]
            print("Saturated rAccel: ", rAccel)
            delta_throttle = previous_rAccel - rAccel
            targetVelocity, velocity_error = calculateVelocityError(Vx, Vy, vCarData, trajectory_matrix, ref_distance, nLap)
            # rAccel, throttle_integral, throttle_prev_error, throttle_P, throttle_I, throttle_D = compute_throttle_pid(velocity_error, timestep, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error)
            driver_command, prev_integral = ff_controller(velocity_error, delta_throttle, throttle_Kp, throttle_Ki, timestep, prev_integral, prev_output)
            # print("Driver Command: ", driver_command)
            rAccel += driver_command
            rAccel = saturation(rAccel, 0, 100)
            previous_rAccel = rAccel
            torque = get_torque(rAccel, nMGU)
            # print("After Torque: ", torque)
            # torque = antiSlip(torque, SlipRL, SlipRR, max_long_slip, slip_constant)
            MDriveRL, MDriveRR = calculateDriveTorque(torque, WheelVelocityRL, WheelVelocityRR)
            MDriveRL = saturation(MDriveRL, 0, max_drive_torque)
            MDriveRR = saturation(MDriveRR, 0, max_drive_torque)
    ...
    return MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, rAccel, brakePressure, throttle_integral, throttle_prev_error, brake_integral, brake_prev_error, targetVelocity, throttle_Kp, throttle_Ki, throttle_Kd, brake_P, brake_I, brake_D, velocity_error