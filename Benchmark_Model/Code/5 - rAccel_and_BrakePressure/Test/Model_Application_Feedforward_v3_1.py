...
# Feedforward integrated with v1_8
...
from steer_control_v3_1_Feedforward import calculate_steering_control_3_1
from pedal_control_v1_8 import calculatePedalPercent
from pedal_control_v3_2_Feedforward import calculatePedalFF
from model_traj_test_Feedforward_v3_0 import plotTraj, plotTrajFF, plot_e_phi_Values, plot_e_l_Values, plot_balance_Values, plot_Angles, plot_Torque, plot_TargetValues, plot_PedalValues, plot_nMGU_Values, plot_Throttle_PID_Values, plot_Brake_PID_Values, plot_vCar_error_Values, plot_refDistances, plot_e_l_Values_FF, plot_Steer_PID_Values
import numpy as np
...
def compute_curvature_from_path(x, y, s):
    dx = np.gradient(x, s)
    dy = np.gradient(y, s)
...
def generate_driving_line_Dev(tracks_df, initial_x, initial_y, yawAngle):
    ...
    tracks_df['curvature'] = compute_curvature_from_path(tracks_df['x'], tracks_df['y'], tracks_df['sDistance'])
    ...
    wrap_df = pd.DataFrame({
        ...
        'y': tracks_df['y'].iloc[:n_wrap].values,
        'Orientation': tracks_df['Orientation'].iloc[:n_wrap].values - 3 * np.pi,
        'curvature': tracks_df['curvature'].iloc[:n_wrap].values
    })
...
class ConsoleApp:
    ...
    def create_parameter_buttons(self):
        ...
        self.ff_var = tk.IntVar()
        self.ff_checkbox = tk.Checkbutton(self.param_frame, text="Enable Feedforward", variable=self.ff_var)
        self.ff_checkbox.grid(row=4, column=3, sticky="w", padx=5)
        ToolTip(self.ff_checkbox, "Uses Feedforward control for steering and pedal control")
    ...
    def create_steer_control_parameters(self):
        ...
        tk.Label(self.steer_control_frame, text="Steer Control Parameters:").grid(row=0, column=0, columnspan=8, sticky="ew")

        self.default_steer_labels = []
        self.default_steer_entries = []
        self.feedforward_labels = []
        self.feedforward_entries = []
        self.common_steer_labels = []
        self.common_steer_entries = []

        # --- Default steer control widgets ---
        label = tk.Label(self.steer_control_frame, text="Angular Offset Gain:")
        label.grid(row=1, column=0, sticky="w")
        self.common_steer_labels.append(label)
        ...
        self.common_steer_entries.append(self.e_phi_gain_entry)
        ToolTip(self.e_phi_gain_entry, "Angular offset gain for heading correction")

        label = tk.Label(self.steer_control_frame, text="Lateral Offset Gains:")
        label.grid(row=2, column=0, sticky="w")
        self.default_steer_labels.append(label)
        ...
        for i, (entry, value) in enumerate(zip(self.lateral_offset_gains_entries, default_values)):
            ...
            self.default_steer_entries.append(entry)
            ToolTip(entry, "Gain for lateral offset correction by lookahead distance")

        label = tk.Label(self.steer_control_frame, text="Individual Lateral Offset Saturations:")
        ...
        label = tk.Label(self.steer_control_frame, text="Total Lateral Offset Saturations:")
        ...
        self.default_steer_entries.append(self.total_lateral_lever_saturations_entry)
        ToolTip(self.total_lateral_lever_saturations_entry, "Total saturation limit for lateral offset control")

        label = tk.Label(self.steer_control_frame, text="Steering Control Saturations:")
        ...
        self.default_steer_entries.append(self.steering_control_saturations_entry)
        ToolTip(self.steering_control_saturations_entry, "Maximum output value from the steering control algorithm")

        ######################################################################################################################################

        # To appear when Feedforward is enabled

        # --- Feedforward widgets (shown only if ff_var == 1) ---

        label = tk.Label(self.steer_control_frame, text="Kp:")
        label.grid(row=2, column=0, sticky="w")
        self.feedforward_labels.append(label)
        self.steer_Kp_entry = tk.Entry(self.steer_control_frame, width=5)
        self.steer_Kp_entry.grid(row=2, column=1, padx=5)
        self.steer_Kp_entry.insert(0, "0.8")
        self.feedforward_entries.append(self.steer_Kp_entry)

        label = tk.Label(self.steer_control_frame, text="Ki:")
        label.grid(row=3, column=0, sticky="w")
        self.feedforward_labels.append(label)
        self.steer_Ki_entry = tk.Entry(self.steer_control_frame, width=5)
        self.steer_Ki_entry.grid(row=3, column=1, padx=5)
        self.steer_Ki_entry.insert(0, "0.5")
        self.feedforward_entries.append(self.steer_Ki_entry)

        label = tk.Label(self.steer_control_frame, text="Kd:")
        label.grid(row=4, column=0, sticky="w")
        self.feedforward_labels.append(label)
        self.steer_Kd_entry = tk.Entry(self.steer_control_frame, width=5)
        self.steer_Kd_entry.grid(row=4, column=1, padx=5)
        self.steer_Kd_entry.insert(0, "0.3")
        self.feedforward_entries.append(self.steer_Kd_entry)

        label = tk.Label(self.steer_control_frame, text="Lateral Offset Gain:")
        label.grid(row=1, column=2, sticky="w")
        self.feedforward_labels.append(label)
        self.e_l_gain_entry = tk.Entry(self.steer_control_frame, width=5)
        self.e_l_gain_entry.grid(row=1, column=3, padx=5)
        self.e_l_gain_entry.insert(0, "2.0")
        self.feedforward_entries.append(self.e_l_gain_entry)

        label = tk.Label(self.steer_control_frame, text="Saturation Lateral Offset:")
        ...
        self.feedforward_entries.append(self.sat_e_l_entry)

        # Callback for visibility based on ff_var
        self.ff_var.trace_add("write", lambda *args: self.update_steer_control_visibility())

        # Apply initial visibility
        self.update_steer_control_visibility()
    ...
    def update_steer_control_visibility(self):
        for label in self.common_steer_labels:
            label.grid()
        for entry in self.common_steer_entries:
            entry.grid()

        if self.ff_var.get() == 1:
            for label in self.default_steer_labels:
                label.grid_remove()
            for entry in self.default_steer_entries:
                entry.grid_remove()
            for label in self.feedforward_labels:
                label.grid()
            for entry in self.feedforward_entries:
                entry.grid()
        else:
            for label in self.feedforward_labels:
                label.grid_remove()
    ...
    def get_track_files(self):
        lap_files = []
        try:
            lap_files = [file for file in os.listdir('TandV') if file.endswith('.csv')]
    ...
    def set_file(self, selected_file):
        self.lap_file = os.path.join('TandV', selected_file)
    ...
    def run_backend_script(self):
        try:
            ...
            def reset_variables():
                ...
                nonlocal t_P_array, t_I_array, t_D_array, b_P_array, b_I_array, b_D_array, s_P_array, s_I_array, s_D_array, saturated_e_l_FF_array, target_steer_array, vCar_error_array
                ...
                s_I_array.clear()
                s_D_array.clear()
                saturated_e_l_FF_array.clear()
                target_steer_array.clear()
            ...
            steer_Kp = float(self.steer_Kp_entry.get())
            steer_Ki = float(self.steer_Ki_entry.get())
            steer_Kd = float(self.steer_Kd_entry.get())
            e_l_gain = float(self.e_l_gain_entry.get())
            ...
            ff_enabled = float(self.ff_var.get())
            if ff_enabled:
                lap_df['rAccel'] = lap_df['rAccel'] * velocity_percentage / 100
                lap_df['pBrakeFIA'] = lap_df['pBrakeFIA'] * velocity_percentage / 100
                # lap_df['xSteerRack'] = lap_df['xSteerRack']
                ...
                try:
                    while nLap < configured_laps:
                        ...
                        if i:
                            ...
                            if i == 1:
                                if ff_enabled:
                                    ...
                                    xSteerRack = lap_df['xSteerRack'].tolist()
                                    sDistance_Pedal = lap_df['sDistance_Pedal'].tolist()
                                    rAccel = lap_df['rAccel'].tolist()
                                    pBrakeFIA = lap_df['pBrakeFIA'].tolist()
                                    vCar_Profile_df = pd.DataFrame({
                                        'sDistance': sDistance,
                                        'vCar': vCar,
                                        'xSteerRack': xSteerRack,
                                        'sDistance_Pedal': sDistance_Pedal,
                                        'rAccel': rAccel,   
                                        'pBrakeFIA': pBrakeFIA
                                    })
                                    vCar_Profile_df['sDistance_Pedal'] = vCar_Profile_df['sDistance_Pedal'] - vCar_Profile_df['sDistance_Pedal'].iloc[0]
                                else:
                                    sDistance = lap_df['sDistance'].tolist()
                                    vCar = lap_df['vCar'].tolist()
                                    vCar_Profile_df = pd.DataFrame({
                        ...
                        if ff_enabled:
                            ...
                            steering_input, e_phi, saturated_e_l, levercoordsplot, idealcoordsplot, error_values, refLine_orientation, refDistance, newSim, nLap = calculate_steering_control_3_1(
                            lap_df, Vx, Vy, yawAngle, x, y, e_phi_gain, timeStep, nLap, newSim, e_l_gains, e_l_limits, total_e_l_limits, steer_limits, Dil_Mode_enabled, max_sDistance_Lap1, clockwise)
                            # else:
                            #     steering_inputs.append(steering_input)
                            #     steering_input, e_phi, saturated_e_l, refLine_orientation, refDistance, steer_integral, steer_prev_error, newSim, s_Kp, s_Ki, s_Kd, target_steer = calculate_steering_FF(
                            #     lap_df, vCar_Profile_df, Vx, Vy, yawAngle, x, y, e_phi_gain, timeStep, nLap, newSim, steer_Kp, steer_Ki, steer_Kd, steer_integral, steer_prev_error, e_l_gain, sat_e_l, Dil_Mode_enabled, max_sDistance_Lap1, clockwise)
                            #     s_P_array.append(s_Kp)
                            #     s_I_array.append(s_Ki)
                            #     s_D_array.append(s_Kd)
                            #     saturated_e_l_FF_array.append(saturated_e_l)
                            #     target_steer_array.append(target_steer)
                            # steering_input, e_phi, saturated_e_l, levercoordsplot, idealcoordsplot, error_values, refLine_orientation, refDistance, newSim, nLap = calculate_steering_control(
                            #     lap_df, Vx, Vy, yawAngle, x, y, e_phi_gain, timeStep, nLap, newSim, e_l_gains, e_l_limits, total_e_l_limits, steer_limits, Dil_Mode_enabled, max_sDistance_Lap1, clockwise)
                            for j in range(8):
                        ...
                        if ff_enabled:
                            print("Enter Pedal FF")
                            MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, rAccel, brakePressure, throttle_integral, throttle_prev_error, brake_integral, brake_prev_error, targetVelocity, t_P, t_I, t_D, b_P, b_I, b_D, velocity_error = calculatePedalFF(lap_df, vCar_Profile_df, 
            ...
            if ff_enabled:
                plot_and_store(self.figures, plot_e_l_Values_FF, base_name, saturated_e_l_FF_array, current_time)
                plot_and_store(self.figures, plot_Steer_PID_Values, base_name, s_P_array, s_I_array, s_D_array, current_time)
                plot_and_store(self.figures, plotTrajFF, base_name, lap_df, x_positions, y_positions, current_time)
            else:
                plot_and_store(self.figures, plot_e_l_Values, base_name, saturated_e_l_values, current_time)
            plot_and_store(self.figures, plot_balance_Values, base_name, steering_inputs, target_steer_array, current_time)
