...
# === Control Barrier Function Based Feedforward ===
# def compute_physics_ff(ref_distance, track_df, vprofile_df, mass = 840, mu=1.7, g = 9.81, s_lookahead=5.0, ds=0.1):
...
#     # print(f"Current Ax: {ax}, Ay: {ay}, Ux: {Ux_now}, Kappa: {kappa_now}")
#     # Fx = ax * mass
#     throttle_percent = np.clip(ax / 16.7 * 100, 0, 100)
#     brake_bar = 0 if ax > 0 else np.clip(abs(ax) * 120 / 9, 0, 120)
#     print(f"Throttle Percent: {throttle_percent}, Brake Bar: {brake_bar}")

#     return throttle_percent, brake_bar

# === Feedforward from Velocity Profile ===
def compute_velocity_profile_ff(Vx, vprofile_df, ref_distance, futureTD, mass):
    speed_interp = interp1d(vprofile_df['sDistance'], vprofile_df['vCar'], fill_value="extrapolate")
    v_now = Vx
    v_future = speed_interp(ref_distance + futureTD) / 3.6
    ax = (v_future - v_now) / futureTD
    Fx = ax * mass
    throttle_percent = np.clip(Fx / 10046 * 100, 0, 100)
    brake_bar = 0 if Fx > 0 else np.clip(abs(Fx) * 120 / 9000, 0, 120)
    return throttle_percent, brake_bar
...
def calculatePedalFF(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
    ...
    rAccel_ff, brake_ff = compute_velocity_profile_ff(Vx, vProfiledf, ref_distance, futureTD, mass)
