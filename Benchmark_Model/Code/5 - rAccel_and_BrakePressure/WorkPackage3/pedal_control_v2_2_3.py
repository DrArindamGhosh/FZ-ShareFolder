...
def compute_throttle_pid(error, dt, Kp, Ki, tracking_signal, integral, prev_error, prev_output):
    """ Compute the PID correction """
    ...
    tracking_signal = tracking_signal * 0.1
    
    rAccel = P + I + tracking_signal
    # rAccel = np.clip(rAccel, 0, 100)
    prev_error = error

    rAccel = saturation(rAccel, -25, 25)

    rAccel = rate_limiter(rAccel, prev_output, dt)
    
    return rAccel, integral, prev_error, P, I, tracking_signal
...
def calculatePedalPercent(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
    ...
    if brakePressure_df < 1:
        if not rAccels:
            rAccels.append(0)
        rAccel_delta = rAccels[-1] - rAccel_df
        print("rAccel_df: ", rAccel_df)
        print("rAccel delta: ", rAccel_delta)
        rAccel, throttle_integral, throttle_prev_error, throttle_P, throttle_I, throttle_D = compute_throttle_pid(velocity_error, timestep, throttle_Kp, throttle_Ki, rAccel_delta, throttle_integral, throttle_prev_error, rAccels[-1])
        print("rAccel after PID: ", rAccel)
        # rAccel = rAccel_df + rAccel_PID # Combine feedforward and PID
        rAccel = rAccel_df + rAccel
        rAccel = np.clip(rAccel, 0, 100)
        print("rAccel after FF: ", rAccel)
