# Attempt not successfully implemented
# Uses throttle cutoff thresholds and lift rates
...
def calculatePedalPercent(tracks_df, vProfiledf, Vx, Vy, timestep, ref_distance, nLap, throttle_Kp, throttle_Ki, throttle_Kd, throttle_integral, throttle_prev_error,
    ...
    rAccel, brakePressure = compute_feedforward(ref_distance, vCarData)
    targetVelocity, velocity_error = calculateVelocityError(Vx, Vy, vCarData, trajectory_matrix, ref_distance, nLap)

    # **NEW LOGIC: IMPLEMENT THROTTLE LIFTING BEFORE BRAKING**
    throttle_cutoff_threshold = -1.5  # Define threshold for coasting before braking
    throttle_lift_rate = 2  # Maximum % reduction per timestep (smooth lift-off)

    if velocity_error >= 0:
        if Dil_Mode_enabled == True:
            if rAccels:
                last_rAccel = rAccels[-1]
            else:
                last_rAccel = rAccel
            ...
            rAccel = np.clip(rAccel + rAccel_PID, 0, 100)  # Combine feedforward and PID
        ...
        if rAccel > 0:
            if rAccels:
                ...
                last_rAccel = rAccel
            rAccel = max(last_rAccel - throttle_lift_rate, 0)  # Reduce gradually
            rAccels.append(rAccel)
            MDriveRL, MDriveRR = get_torque(rAccel, nMGU), get_torque(rAccel, nMGU)  # Apply reduced torque
            brakePressure = 0  # No brakes yet
            MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR = 0, 0, 0, 0
            throttle_P = 0
            throttle_I = 0
            throttle_D = 0
            throttle_integral = 0
            ...
            brake_D = 0 
            print("Throttle Lift")        
        elif velocity_error < throttle_cutoff_threshold:
            if Dil_Mode_enabled == True:
                ...
                brakePressure = np.clip(brakePressure + brakePressure_PID, 0, 120)  # Combine feedforward and PID
                prev_bP = brake_P
                prev_bI = brake_I
                prev_bD = brake_D
                rAccel = 0
            ...
            # **If within tolerance, just coast (No throttle, No brakes)**
            brakePressure = 0
            MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR = 0, 0, 0, 0
            MDriveRL, MDriveRR = 0, 0
            rAccel = 0
            throttle_P = 0
            throttle_I = 0
            throttle_D = 0
