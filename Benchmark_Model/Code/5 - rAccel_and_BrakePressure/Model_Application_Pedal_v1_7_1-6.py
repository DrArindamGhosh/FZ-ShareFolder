...
# Merging live data displays with map and dials with new graph display logic
...
class ConsoleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("V3M Simulation Console")
        ...
        # Initialize fullscreen window for graph display
        self.fullscreen_window = None
        self.graph_file_list = []
        self.current_graph_index = 0
        self._append_to_console_output_safe = print  # Mocking the console output method
    ...
    def create_control_buttons(self):
        ...
        self.start_sim_button = tk.Button(button_row, text="START SIM", bg="green", command=self.start_simulation)
        self.stop_button = tk.Button(button_row, text="STOP SIM", bg="red", command=self.stop_simulation)
        self.reset_button = tk.Button(button_row, text="RESET SIM", bg="light blue", command=self.reset_simulation_button)
        ...
        def populate_checkboxes():
            ...
            for graph_file in all_graphs:
                if graph_file not in current_files:
                    ...
                    try:
                        img_path = os.path.join(folder_path, graph_file)
                        img = Image.open(img_path)
                        img.thumbnail((100, 75))
                        thumb = ImageTk.PhotoImage(img)
                        thumbnail_refs[graph_file] = thumb

                        img_label = ttk.Label(row_frame, image=thumb)
                        img_label.pack(side="left", padx=10)
                        img_label.image_ref = thumb  # ⬅️ Prevent garbage collection

                        # ---- Hover Zoom ----
                        def show_zoom(event, path=img_path):
                            try:
                                zoom_img = Image.open(path)
                                zoom_img.thumbnail((300, 225))
                                zoom_photo = ImageTk.PhotoImage(zoom_img)
                                zoom_popup = tk.Toplevel(graphs_window)
                                zoom_popup.overrideredirect(True)
                                zoom_popup.geometry(f"+{event.x_root+10}+{event.y_root+10}")
                                zoom_label = ttk.Label(zoom_popup, image=zoom_photo)
                                zoom_label.image = zoom_photo
                                zoom_label.pack()
                                zoom_popup.image_ref = zoom_photo  # ⬅️ Prevent garbage collection
                                img_label.zoom_popup = zoom_popup
                            except Exception as e:
                                self._append_to_console_output_safe(f"Zoom error: {e}\n")

                        def hide_zoom(event):
                            if hasattr(img_label, "zoom_popup"):
                                img_label.zoom_popup.destroy()
                                del img_label.zoom_popup

                        img_label.bind("<Enter>", show_zoom)
                        img_label.bind("<Leave>", hide_zoom)

                        # ---- Double-click Fullscreen Viewer ----
                        def open_fullscreen(path, title):
                            try:
                                # Ensure fullscreen_window attribute exists and is valid
                                if not hasattr(self, 'fullscreen_window') or self.fullscreen_window is None or not self.fullscreen_window.winfo_exists():
                                    self.fullscreen_window = tk.Toplevel(self.root)
                                else:
                                    # clear out previous content
                                    for w in self.fullscreen_window.winfo_children():
                                        w.destroy()

                                top = self.fullscreen_window
                                top.title(f"Full-Screen View: {title}")
                                ...
                                def save_as():
                                    ...
                                    if dst:
                                        img.save(dst)
                                        self._append_to_console_output_safe(f"Graph saved as: {dst}\n")

                                ttk.Button(top, text="Save As…", command=save_as).pack(pady=10)

                                top.focus_set()
                            except Exception as e:
                                self._append_to_console_output_safe(f"Open full-screen error: {e}\n")

                        #img_label.bind("<Double-Button-1>", open_fullscreen)
                        img_label.bind("<Double-Button-1>", lambda e, p=img_path, t=graph_file: open_fullscreen(p, t))
                    ...
                    while nLap < configured_laps:
                        ...
                        if i:
                            ...
                            if i == 1:
                                ...
                                track_x_positions = tracks_df['x'].tolist()
                                track_y_positions = tracks_df['y'].tolist()
