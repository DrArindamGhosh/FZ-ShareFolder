...
- Utility Functions:
    ...
    - generate_driving_line_DiL / generate_driving_line_Dev: Prepares and processes track data for DiL (Driver-in-the-Loop) and Offline (Development) modes.
- BackgroundTask: 
...
- Support for both DiL and Offline simulation modes.
- Error handling and user feedback via the console output.
Note:
- The script expects the "Trajectories" folders.
- External modules (e.g., steer_control_v1_8, pedal_control_v1_8, model_traj_test_v8) are required for full functionality.
"""
...
import logging
import traceback
from steer_control_v1_8 import calculate_steering_control
from pedal_control_v1_9 import calculatePedalPercent
from model_traj_test_v8 import plotTraj, plot_e_phi_Values, plot_e_l_Values, plot_balance_Values, plot_Angles, plot_Torque, plot_TargetValues, plot_PedalValues, plot_nMGU_Values, plot_Throttle_PID_Values, plot_Brake_PID_Values, plot_vCar_error_Values, plot_refDistances
import numpy as np
...
ERROR_REPORT_PATH = os.path.join(LOG_DIR, "simulation_errors.txt")
...
logger = logging.getLogger("V3M")

def report_error(context: str, exc: Exception) -> None:
    """
    Log full traceback to main log and also append a human-readable line to a dedicated error report.
    """
    logger.exception("Exception in %s: %s", context, exc)
    try:
        with open(ERROR_REPORT_PATH, "a", encoding="utf-8") as f:
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Context: {context}\n"
                f"Error: {repr(exc)}\n"
                f"Traceback:\n{traceback.format_exc()}\n"
                f"{'-'*72}\n"
            )
    except Exception as write_err:
        logger.error("Failed to write to error report file: %s", write_err)
...
def generate_driving_line_DiL(tracks_df, yawAngle):
    logger.debug("Generating DiL driving line...")
    x = tracks_df['xCarX']
    y = tracks_df['xCarY']

    orientation = calculate_orientation(x, y)
    logger.debug("Orientation calculated (DiL).")

    tracks_df['Orientation'] = orientation

    sDistance, x, y, orientation = resample_uniformly(tracks_df, 'sLapDistance', 'xCarX', 'xCarY', 'Orientation', num_points=1500)
    ...
    logger.debug("DiL driving line generated. clockwise=%s, max_sDistance=%.3f", clockwise, max_sDistance_lap1)
    return tracks_df, max_sDistance_lap1, clockwise

def generate_driving_line_Dev(tracks_df, initial_x, initial_y, yawAngle):
    logger.debug("Generating Dev (offline) driving line...")
    x = tracks_df['xCarX']
    y = tracks_df['xCarY']

    orientation = calculate_orientation(x, y)
    logger.debug("Orientation calculated (Dev).")

    tracks_df['Orientation'] = orientation

    tracks_df['sLapDistance'] = tracks_df['sLapDistance'] - tracks_df['sLapDistance'].iloc[0]
    ...
    logger.debug("Dev driving line generated. clockwise=%s, max_sDistance=%.3f", clockwise, max_sDistance_lap1)
    return tracks_df, max_sDistance_lap1, clockwise
...
class BackgroundTask():
    ...
    def start(self):
        if not self._is_running:
            ...
            logger.info("BackgroundTask started: %s", self._task_func.__name__)
            self._worker_thread.start()

    def stop(self):
        logger.info("BackgroundTask stop requested: %s", getattr(self._task_func, "__name__", "unknown"))
        self._is_running = False
    ...
    class WorkerThread(threading.Thread):
        ...
        def run(self):
            try:
                ...
                report_error(f"BackgroundTask::{self._task_func.__name__}", e)
            self._bg_task.stop()
...
class TkRepeatingTask():
    ...
    def start(self):
        ...
        logger.debug("TkRepeatingTask started every %d ms for %s", self.__freq, self.__func.__name__)
        self.__on_timer()

    def stop(self):
        self.__is_running = False
        logger.debug("TkRepeatingTask stopped for %s", self.__func.__name__)
...
class ConsoleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("V3M Simulation Console App")
        logger.info("ConsoleApp initialized")
        ...
        self.canvas = tk.Canvas(self.root, width=900, height=950)
    ...
    def create_ip_port_input(self):
        ...
        self.ip_var = tk.StringVar(ip_port_frame)
        self.ip_var.set("192.168.100.60")
        self.ip_entry = tk.OptionMenu(ip_port_frame, self.ip_var, "192.168.100.60", "127.0.0.1")
        self.ip_entry.config(width=15)
    ...
    def create_parameter_buttons(self):
        ...
        self.unlimited_laps_var = tk.IntVar(value=1)
        ...
        # DiL Mode and Offline Mode (mutually exclusive)
        ...
        self.invert_coords_checkbox = tk.Checkbutton(self.param_frame, text="Offline Mode (Invert X and Y)", variable=self.invert_coords_var)
    ...
    def create_force_input_buttons(self):
        ...
        # --- Force Input Fields for Offline Mode ---
    ...
    def create_pedal_control_parameters(self):
        ...
        tk.Label(self.pedal_control_frame, text="Max Brake Pressure:").grid(row=4, column=2, sticky="w")
        self.max_brake_pressure_entry = tk.Entry(self.pedal_control_frame, width=5)
        self.max_brake_pressure_entry.grid(row=4, column=3, padx=5)
        self.max_brake_pressure_entry.insert(0, "120")
        ToolTip(self.max_brake_pressure_entry, "Maximum brake pressure allowed (bar)")
    ...
    def create_live_data_frame(self):
        ...
        def export_laps():
            ...
            try:
                ...
                logger.info("Lap times exported to %s", export_path)
            except Exception as e:
                report_error("export_laps", e)
    ...
    def plot_track_on_map(self, x_positions, y_positions):
        logger.debug("Plotting track on mini-map with %d points", len(x_positions))
    ...
    def create_console_output(self):
        self.console_output = tk.Text(self.scrollable_frame, height=15, width=50, bg="black", fg="white")
    ...
    def run_server(self):
        if self.abort_flag.is_set():
            ...
            logger.warning("Server start aborted (abort_flag set)")
            return
        try:
            ...
            logger.info("Server bound to %s:%d", SERVER_IP, SERVER_PORT)
            ...
            report_error("run_server", e)
        logger.info("Server stopped.")

    def start_simulation(self):
        if not self.client_address:
            self._append_to_console_output_safe("No client connected. Start the server first.\n")
            logger.warning("Start simulation requested without client connection")
            return
        self._append_to_console_output_safe("Start SIM pressed\n")
        logger.info("Simulation starting...")
        # Disable all widgets in param_frame except the Velocity dropdown
        for child in self.param_frame.winfo_children():
            # Check if the widget is the Velocity dropdown
            if child == self.num_Velocity_dropdown:
                child.config(state="normal")
            elif isinstance(child, (tk.Entry, tk.OptionMenu, tk.Checkbutton, tk.Button)):
                child.config(state="disabled")
    ...
    def abort_simulation(self):
        ...
        logger.warning("Simulation abort pressed")
        self.abort_flag.set()
        ...
        try:
            if not self.simulation_task.is_running() and self.server_socket:
                self.server_socket.close()
                logger.info("Server socket closed on abort")
    ...
    def reset_simulation_task(self):
        logger.info("Simulation reset initiated")
        self.reset_flag.set()
    ...
    def reset_param_GUI(self):
        ...
        logger.info("Parameter GUI reset to defaults")
    ...
    def reset_steer_control_GUI(self):
        ...
        self.steering_control_saturations_entry.insert(0, "28")
        logger.info("Steer control parameters reset")
    ...
    def reset_pedal_control_GUI(self):
        ...
        self.futureTD.insert(0, "5")
        logger.info("Pedal control parameters reset")
    ...
    def reset_torque_control_GUI(self):
        ...
        self.max_brake_torque_entry.insert(0, "3500")
        logger.info("Torque control parameters reset")

    def reset_GUI_variables(self):
        # Reset all the variables used in the simulation to their default values
        logger.debug("Resetting GUI runtime variables (force inputs and torque params)")
        # Force driver inputs
    ...
    def validate_csv_file(self):
        ...
        try:
            ...
            logger.info("Track file read successfully: %s (rows=%d, cols=%d)", self.lap_file, len(df), len(df.columns))
        except Exception as e:
            self._append_to_console_output_safe(f"Error reading selected file: {e}\n")
            report_error("validate_csv_file", e)
            return False
    ...
    def get_track_files(self):
        lap_files = []
        trajectories_dir = self.find_trajectories_folder()
        if trajectories_dir:
            try:
                lap_files = [file for file in os.listdir(trajectories_dir) if file.endswith('.csv')]
            except Exception as e:
                self._append_to_console_output_safe(f"Error: {e}\n")
        else:
            self._append_to_console_output_safe("Error: 'Trajectories' folder not found.\n")
        return lap_files
    
    def find_trajectories_folder(self):
        # Search upwards from current file location for 'Trajectories' folder
        current_dir = os.path.dirname(os.path.abspath(__file__))
        while True:
            candidate = os.path.join(current_dir, 'Trajectories_New')
    ...
    def set_file(self, selected_file):
        self.lap_file = os.path.join('Trajectories_New', selected_file)
        self._append_to_console_output_safe(f"Selected Track File: {self.lap_file}\n")
        logger.info("Track file selected: %s", self.lap_file)
    ...
    def show_graphs_from_folder(self):
        ...
        if not os.path.exists(folder_path):
            ...
            logger.warning("Graphs folder not found: %s", folder_path)
            return
        ...
        def populate_checkboxes():
            ...
            for graph_file in all_graphs:
                ...
                try:
                    ...
                    report_error("show_graphs_from_folder::thumbnail", e)
        ...
        def display_selected_graphs():
            ...
            logger.info("Displaying %d selected graphs", len(selected_files))
            ...
            for file in selected_files:
                try:
                    ...
                    report_error("show_graphs_from_folder::display_selected_graphs", e)
    ...
    def start_reset_listener(self):
        def listen_for_reset():
            ...
            try:
                ...
                logger.info("Reset listener bound on UDP port 6015")
                while True:
                    data, addr = reset_socket.recvfrom(1)
                    if data == b'\x00':  # Single byte message with value zero
                        logger.info("External reset command received from %s", addr)
                        self.reset_simulation_task()
            except Exception as e:
                report_error("start_reset_listener", e)
            finally:
                try:
                    reset_socket.close()
                except Exception:
        ...
        self.reset_listener_thread = threading.Thread(target=listen_for_reset, daemon=True, name="ResetListener")
        self.reset_listener_thread.start()

    
    def run_backend_script(self):
        try:
            logger.info("Backend simulation loop starting")
            i = 0
            nLap = 0
            lastNLap = 0
            LapCompleted = False
            ...
            if not self.validate_csv_file():
                logger.error("CSV validation failed; aborting backend script")
                return

            self._update_connection_status_safe("Connected")
            self._append_to_console_output_safe("Starting simulation...\n")
            try:
                raw_trajectory = pd.read_csv(self.lap_file)
                logger.info("Trajectory loaded: %s (rows=%d)", self.lap_file, len(raw_trajectory))
            except Exception as e:
                report_error("run_backend_script::read_trajectory", e)
                return
            ...
            try:
                ...
                max_brake_pressure = float(self.max_brake_pressure_entry.get())
                ...
                Dil_Mode_enabled = bool(self.DiL_Mode_var.get())
                logger.info(
                    "Parameters loaded | Dil=%s, laps=%s, e_phi_gain=%.3f, steer_limits=%.3f",
                    Dil_Mode_enabled, configured_laps, e_phi_gain, steer_limits
                )
            except Exception as e:
                report_error("run_backend_script::read_parameters", e)
                return

            if Dil_Mode_enabled == 0:
            ...
            while not self.abort_flag.is_set():
                ...
                logger.info("Waiting for client loop entered")
                ...
                try:
                    while nLap < configured_laps:
                        ...
                        if i:
                            if self.abort_flag.is_set():
                                ...
                                self.clear_lap_times()
                                self.abort_flag.clear()
                                logger.warning("Abort flag detected during simulation loop; breaking")
                                break
                            if self.reset_flag.is_set():
                                ...
                                logger.info("Reset flag handled; restarting iterations")
                                break
                            ...
                            if Dil_Mode_enabled:
                                ...
                                data_to_send = struct.pack('fffffff', steering_input, 0.0, 0.0, MDriveRL, MDriveRR, MBrakeFL, MBrakeFR)
                            ...
                            try:
                                ...
                                if received_length == expected_length:
                                    ...
                                    logger.error("Socket packet length mismatch: got=%d expected=%d", received_length, expected_length)
                                    break
                            except socket.timeout:
                                self._append_to_socket_output_safe("Socket timeout: No response from client\n")
                                logger.warning("Socket timeout while awaiting client response")
                            except Exception as e:
                                self._append_to_socket_output_safe(f"Socket error: {e}\n")
                                report_error("run_backend_script::socket_io", e)

                            if i == 1:
                                try:
                                    sDistance = raw_trajectory['sLapDistance'].tolist()
                                    vCar = raw_trajectory['vCar'].tolist()
                                    vCar_Profile_df = pd.DataFrame({
                                        'sDistance': sDistance,
                                        'vCar': vCar,
                                    })
                                    velocity_percentage = int(self.num_Velocity_var.get())
                                    vCar_Profile_df['vCar'] = vCar_Profile_df['vCar'] * velocity_percentage / 100
                                    vCar_Profile_df['sDistance'] = vCar_Profile_df['sDistance'] - vCar_Profile_df['sDistance'].iloc[0]
                                    if Dil_Mode_enabled == True:
                                        lap_df, max_sDistance_Lap1, clockwise = generate_driving_line_DiL(raw_trajectory, yawAngle)
                                    if Dil_Mode_enabled == False:
                                        lap_df, max_sDistance_Lap1, clockwise = generate_driving_line_Dev(raw_trajectory, x, y, yawAngle)
                                    ...
                                    logger.info("Lap reference generated (first iteration). clockwise=%s, maxS=%.2f", clockwise, max_sDistance_Lap1)
                                    newSim = True
                                except Exception as e:
                                    report_error("run_backend_script::first_iteration_setup", e)
                                    break

                        if velocity_percentage != int(self.num_Velocity_var.get()):
                            velocity_percentage = int(self.num_Velocity_var.get())
                            vCar_Profile_df['vCar'] = raw_trajectory['vCar'] * velocity_percentage / 100
                            vCar_Profile_df['sDistance'] = raw_trajectory['sLapDistance'] - raw_trajectory['sLapDistance'].iloc[0]
                            self._append_to_console_output_safe(f"Velocity percentage changed to {velocity_percentage}%\n")
                            logger.info("Velocity percentage changed to %d%%", velocity_percentage)
                        ...
                        if lastNLap < nLap:
                            ...
                            logger.info("Lap %d completed in %.3fs", nLap, lap_time)
                        lastNLap = nLap
                        ...
                        if vCar > 300:
                            logging.error("Unrealistic vehicle speed detected: vCar=%.2f m/s; breaking simulation loop", vCar)
                            self._append_to_console_output_safe(f"Unrealistic vehicle speed detected: vCar={vCar:.2f} m/s; breaking simulation loop\n")
                            break
                        ...
                        try:
                            steering_input, e_phi, saturated_e_l, levercoordsplot, idealcoordsplot, error_values, refLine_orientation, refDistance, newSim, nLap, abort_e_l = calculate_steering_control(
                            ...
                            report_error("calculate_steering_control", e)
                            break
                        ...
                        if auto_abort:
                            if abort_e_l > 30 or abort_e_l < -30:
                                self._append_to_console_output_safe(f"Vehicle has left the track\n")
                                logger.warning("Auto-abort triggered: e_l=%.3f", abort_e_l)
                                break
                        ...
                        try:
                            MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, rAccel, brakePressure, throttle_integral, throttle_prev_error, brake_integral, brake_prev_error, targetVelocity, t_P, t_I, t_D, b_P, b_I, b_D, velocity_error = calculatePedalPercent(
                                lap_df, vCar_Profile_df,
                                ...
                                max_engine_brake_torque, differential_gain_1, driving_differential_gain, overrun_differential_gain, max_drive_torque, max_brake_torque, futureTD, StopFlag, rAccel, Dil_Mode_enabled, max_brake_pressure
                            )
                        except Exception as e:
                            report_error("calculatePedalPercent", e)
                            break
                        ...
                        if force_controls == True:
                            try:
                                ...
                                report_error("force_controls_parse", e)
                                force_controls = False
                                logger.warning("Invalid forced input values; reverting to V3M control")
                        else:
                            MDriveFL = 0
                            MDriveFR = 0
                        ...
                        if self.stop_flag.is_set():
                            ...
                            StopFlag = True
                            logger.info("Stop flag set; commanding safe stop torques/pressures")
                            if Dil_Mode_enabled:
                                rAccel = 0
                                brakePressure = max_brake_pressure
                            else:
                                rAccel = 0
                                brakePressure = max_brake_pressure
                                MDriveFL = 0
                    ...
                    logger.info("Client connection closed / final braking command sent")
            ...
            try:
                ...
                report_error("run_backend_script::final_send", e)

            received_length = len(data) if 'data' in locals() else 0
            expected_length = 16 * 4
            output_text = (f":::::Hard Braking:::::\n\n")
            self._append_to_console_output_safe(output_text)
            ...
            timer_reset += lap_time
            self.update_lap_label_with_times(nLap, lap_time)
            logger.info("Final lap time recorded: %.3fs", lap_time)

            try:
                self.server_socket.close()
                logger.info("Server socket closed")
            except Exception as e:
                report_error("run_backend_script::socket_close", e)

            def plot_and_store(figures, plot_func, *args):
                """
                Helper to call a plotting function and store its figure in the list.
                Logs any exceptions and continues gracefully.
                """
                try:
                    fig = plot_func(*args)
                    if fig is not None:
                        figures.append(fig)
                        logger.debug("Plot created successfully: %s", plot_func.__name__)
                    else:
                        logger.warning("Plot function %s returned None", plot_func.__name__)
                except Exception as e:
                    report_error(plot_func.__name__, e)
                    self._append_to_console_output_safe(f"Error plotting {plot_func.__name__}: {e}\n")

            # --- Plotting and wrap-up phase ---
            logger.info("Generating plots...")

            expected_plots = 12 + (1 if (lever_coordinates and ideal_coordinates) else 0)

            if lever_coordinates and ideal_coordinates:
                plot_and_store(self.figures, plotTraj, base_name, lap_df, x_positions, y_positions,
                               levercoordsplot, idealcoordsplot, error_values, current_time)
            ...
            logger.info("Plot generation complete: %d/%d figures created", len(self.figures), expected_plots)

            self._append_to_console_output_safe("Figures saved locally.\n")
            logger.info("Figures saved locally (count=%d)", len(self.figures))

            self._append_to_console_output_safe("Script finished execution.\n")
            logger.info("Backend script finished execution; updating UI state to 'Disconnected'")
            ...
            report_error("run_backend_script", e)

    # --- Displaying generated figures inside a new window ---
    def display_graphs(self):
        if not self.figures:
            self._append_to_console_output_safe(
                "No graphs available to display. Run the simulation first.\n"
            )
            logger.warning("Display graphs requested but no figures are available")
            return

        logger.info("Opening graphs window with %d figure(s)", len(self.figures))
        graphs_window = tk.Toplevel(self.scrollable_frame)
        ...
        for fig in self.figures:
            try:
                ...
                logger.debug("Displayed figure on canvas successfully")
            except Exception as e:
                report_error("display_graphs::figure_display", e)
                self._append_to_console_output_safe(f"Error displaying a figure: {e}\n")
    ...
    def close(self):
        ...
        try:
            if self.simulation_task.is_running():
                logger.debug("Stopping simulation task")
                self.simulation_task.stop()
            if self.reset_task.is_running():
                logger.debug("Stopping reset task")
                self.reset_task.stop()
            if self.refresh_task.is_running():
                logger.debug("Stopping console refresh task")
                self.refresh_task.stop()
            if self.server_socket:
                try:
                    self.server_socket.close()
                    logger.debug("Server socket closed during shutdown")
                except Exception as e:
                    report_error("close::server_socket_close", e)
        finally:
            self.scrollable_frame.quit()
            logger.info("Tkinter main loop quit")
    ...
    def update_dials(self):
        try:
            ...
            report_error("update_dials", e)

        self.root.after(50, self.update_dials)


def run_app():
    logger.info("Launching V3M Simulation Console Application...")
    root = tk.Tk()
    app = ConsoleApp(root)
    ...
    try:
        ...
        report_error("run_app::mainloop", e)
    finally:
