...
gearRatioFront = 8.47 
maxEngineBrakeTorque = 1
diffTorqueTransferGain1 = .2            # To be tweaked
diffTorqueTransferGain2 = .2
...
def calculateTargetVelocity(vCarData, trajectory_matrix, targetDistance, steering_input):
    ...
    targetVelocityComparison = targetVelocity / 10
    compareVelocities.append(targetVelocityComparison)
    ...
    return targetVelocity, targetVelocityComparison


# def calculateScalingFactor(trajectory_matrix, refDistance, x, y, yawAngle, Vx, Vy):

#     k_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 1], kind='linear', fill_value="extrapolate")
#     phi_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 2], kind='linear', fill_value="extrapolate")
#     x_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 3], kind='linear', fill_value="extrapolate")
#     y_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 4], kind='linear', fill_value="extrapolate")
...
#     distance = (y - traj_y)*np.cos(phi) - (x - traj_x)*np.sin(phi)
#     radius = 1/k

#     scalingFactor = (1 - distance/radius)/(Vx*np.cos(yawAngle - phi) - Vy*np.sin(yawAngle - phi))
...
def antiLockBrakes(longThrust, LFslip, RFslip, LRslip, RRslip):
    ...
    antiLockRR = 0.5 * np.sin(np.arctan(constantB * (RRslip + maxSlipL))) + 0.5                                                                       # Eq. 5.6
...
def calculateTorqueControl(Vx, Vy, yawAngle, x, y, WheelVelocityRL, WheelVelocityRR, SlipFL, SlipFR, SlipRL, SlipRR, steering_input):
    ...
    targetVelocity, targetVelocityComparison = calculateTargetVelocity(vCarData, trajectory_matrix, targetDistance, ref_distance)
    ...
    return MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, targetVelocity, longAcceleration, slipThrust, oldLongThrust, targetVelocityComparison