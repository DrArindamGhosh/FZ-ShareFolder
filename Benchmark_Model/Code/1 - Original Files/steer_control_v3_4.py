...
# mat_data = loadmat('Adjusted_MEX_driving_line.mat')
...
def CorrespondingIdealPathPoints(trajectory_matrix, lever_distances, refDistance):
    ...
    xr_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 3], kind='linear', fill_value="extrapolate")
    yr_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 4], kind='linear', fill_value="extrapolate")
...
def AngularOffset(trajectory_matrix, refDistance, orientation, e_phi_gain):
    ...
    refLine_orientation = []
    phi_interpolator = interp1d(trajectory_matrix[:, 0], trajectory_matrix[:, 2], kind='linear', fill_value="extrapolate") # Equation 4.25
...
def calculate_steering_control(Vx, Vy, yawAngle, x, y):
    ...
    return steering_control_input, e_phi, saturated_e_l, lever_coords_plot, ideal_coordinates_plot, e_l, refLine_orientation
