...
from steer_control_v4_Monaco import calculate_steering_control
from speed_control_v3_Monaco import calculateTorqueControl
...
Monaco_Traj_Mat = 'Monaco\Monaco_Traj_Mat.xlsx'
trajectory_matrix = pd.read_excel(Monaco_Traj_Mat)
trajectory_matrix.head(), trajectory_matrix.columns

MonacoVelocityProfile = 'Monaco\MonacoVelocityTest.xlsx'
MonacoVelocity = pd.read_excel(MonacoVelocityProfile)
MonacoVelocity.head(), MonacoVelocity.columns
...
def main():
    ...
    for _ in range(iterations):
        ...
        if nLaps:
            ...
            print(f"Last Lap: {lastNLap} ")
            if lastNLap < nLap:
                print(f"Lap Completed at iteration {_+1}")
                LapCompleted = True
                # yawAngle = yawAngle + (nLap * 2*np.pi)
            lastNLap = nLaps[-1]
        else:
            lastNLap = nLap
        ...
        steering_input, e_phi, saturated_e_l, levercoordsplot, idealcoordsplot, error_values, refLine_orientation, refDistance = calculate_steering_control(trajectory_matrix, Vx, Vy, yawAngle, x, y)
        # if abs(e_phi) > 5:
        ...
        MDriveRL, MDriveRR, MBrakeFL, MBrakeFR, MBrakeRL, MBrakeRR, targetVelocity, longAcceleration, slipThrust, oldLongThrust, nLap = calculateTorqueControl(
                        trajectory_matrix, MonacoVelocity, Vx, Vy, yawAngle, x, y, rearLHSWheelVelocity,
                        rearRHSWheelVelocity,
                        frontLHSWheelSlipRatio, frontRHSWheelSlipRatio, rearLHSWheelSlipRatio, rearRHSWheelSlipRatio,
                        steering_input
                    )
