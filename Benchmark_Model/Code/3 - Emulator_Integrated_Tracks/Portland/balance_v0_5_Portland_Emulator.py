from steer_control_v3_4_Portland import calculate_steering_control
from speed_control_v2_3_Portland import calculateTorqueControl
...
def main():
    ...
    for _ in range(iterations):
        ...
        #     if yawAngle > 0: 
        #         yawAngle = yawAngle - 2*np.pi
        #         if yawAngles and yawAngles[-1] < -3:
        #             if yawAngle > -2: 
        #                 yawAngle = yawAngle - 2*np.pi
        
        # if yawAngles and yawAngles[-1] < -4.5:
        #     if yawAngle > -4: 
        #         yawAngle = yawAngle - 2*np.pi
        ...
        if yawAngles and yawAngles[-1] < -4:
            if yawAngle > -4: 
                yawAngle = yawAngle - np.pi
